...
> **Pendiente de esta ronda, y no es del arreglo:** la comprobación **a mano** contra el comando de verdad
> —`migrate-tenants --revert` sin `--yes` sobre la base de trabajo— no se pudo ejecutar: el *hook* de Bash
> del entorno dejó de responder a mitad de la ronda. Lo que sí está medido es el nivel 1 de la guarda y la
> batería entera. Es la única línea de esta ronda que no se verificó como A-4 verificó la suya, y queda
> escrita para no darla por hecha.
