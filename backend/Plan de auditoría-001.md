...
> · **Añadido al cerrar el bloque:** el ***spec*** y **el código generado**. La costura 4 no se contesta con
> el YAML —declara `403` en 44 operaciones, eso no dice qué se generó— sino con `Operations.…Output`, que
> exige un **`swift build` primero** porque `APIContract` no existe en disco hasta que corre el plugin
> (`D-69`); el fichero es
> `.build/plugins/outputs/backend/APIContract/destination/OpenAPIGenerator/GeneratedSources/Types.swift`. Y
> **hace falta el servidor levantado**, que el bloque no presupuestaba: las tres afirmaciones falsas que
> encontró solo se caen ejecutándolo. El montaje es corto y **sin red** —`docker compose up -d` ·
> `provision-tenant <scratch>` · `delete from club_<scratch>.clubs` y, para la otra mitad,
> `drop table club_<scratch>.clubs`—: nada de `seed-competition`, que es lo que encarecía a A-4.
...
4. **Heredado de A-5, y es un patrón antes que un hallazgo** (H-38, con H-17 y H-26 detrás). Tres veces ha
   ...
   nueva.
5. **Heredado de A-6, y es la cuarta instancia del mismo patrón en su forma más barata** (H-40, H-42). A-6
   contestó sus tres costuras buenas con `grep` y con el código generado, y **las tres afirmaciones falsas
   que encontró solo se cayeron ejecutando el servidor**: dos `curl` contra un tenant saboteado. O sea que
   el corolario de H-38 tiene una versión más afilada — **lo que `grep` demuestra es que algo está, no que
   haga lo que dice**. Los dos huecos que encontró —la traducción de lo que se escapa de un *handler* y
   `tenantMismatch`— **los tapó su propia ronda** (`ErrorBoundaryTests`), así que lo que llega aquí no es
   deuda sino dos lecturas:
   - **Una corrección al corolario, y es la más útil de las dos.** La mutación del log se declaró *"fuera del
     arnés por construcción"* —como los códigos de salida de H-39— y **era falso**: `APIHandler` ya inyecta
     su `Logger` por este mismo motivo (H-27) y `CapturingLogHandler` ya existía en el *target*, así que se
     mató sin tocar producción. **Antes de clasificar algo como no-automatizable, buscar el caso hermano.**
     A-7 tiene que aplicar esto a la lista que hereda: de *"lo que solo se comprueba a mano"*, ¿cuánto es de
     verdad imposible y cuánto es que nadie fue a mirar?
   - **Y la forma que sí resistió:** el código de salida de un proceso (H-39) y `DetachedBackgroundWork` (A-4).
     Ésos siguen fuera, y conviene que A-7 diga **por qué** en una línea cada uno, para que la categoría
     tenga borde en vez de ser un cajón.
...
| **H-07** | **A-7** | **S2** | **Un verde omitido y un verde real son indistinguibles en la línea de resumen.** Con Postgres parado, `swift test` omite los niveles 3 y 4 y remata con `✔ Test run with 266 tests in 40 suites passed` — **el mismo texto, el mismo recuento** que cuando sí corren, porque el total sale de la lista de tests y no de lo ejecutado. La única señal es **la duración**: 0,10 s omitiendo contra 4,57 s de verdad. Los avisos de omisión existen y son buenos (§5.2 del README), pero quedan sepultados y el renglón que uno mira es el último. **Lo encontró tropezando: al cerrar A-0 se dio por verde una pasada de 0,1 s.** Anotado a A-7 porque su remedio vive ahí —CI con `REQUIRE_DB=1`— y porque refuerza su S2: la guarda está escrita y desconectada | `swift test` con `docker compose stop db` (0,10 s, «passed») frente a `REQUIRE_DB=1 swift test` en las mismas condiciones (falla y dice por qué) | **Confirmado y acotado por A-7, y con remedio escrito · el *workflow* es deber de despliegue, con el cron.** Reproducido con los **297** tests de hoy y el resultado es **peor** de lo que este renglón decía: los seis renglones de resumen son **idénticos en texto y en recuento** corriendo y omitiendo —`7/60/51/105/50/24` en los dos casos, porque el total sale de la lista—, y **lo único que cambia es la duración**: 5,50 s y 2,70 s en los dos *targets* de BD contra **0,002 s y 0,001 s** omitiendo. `swift test` con la base parada sale **0**; con `REQUIRE_DB=1`, **1**, con el motivo en una línea. **Lo que A-7 aporta es que no hay que leer texto**: `swift test --xunit-output` emite un XML con los seis *targets* como `<testsuite>`, cada uno con `tests`/`failures`/`skipped`, los **297** `<testcase>` y **el motivo del omitido dentro de `<skipped>`**. Ver el renglón de cierre de A-7 en §7 |
...
| **H-39** | A-5 | **S3** | **Los comandos no fallan: revientan — y el mensaje sale dos y tres veces.** Lo encontró **la comprobación a mano de la ronda de A-5**, no el bloque. Las dos salidas de error del comando funcionan y dicen lo correcto —la guarda de H-32 (*"repítelo con `--yes`"*) y el fallo atribuido de H-33 (*"No se pudo migrar el club 'a5v' … `42P07`"*)— pero **las dos terminan en `Fatal error: Error raised at top level` y código de salida `133`** (128+5, `SIGTRAP`), con el mensaje impreso **tres veces**: dos como `WARNING` de ConsoleKit y una en el volcado del *crash*. Un `133` no es *"falló como estaba previsto"*: es la firma de un programa que se ha caído, y quien lo vea en un log de despliegue no puede distinguir la guarda que hizo su trabajo de un *bug*.<br><br>**Y alcanza a los cinco comandos, `ingest` incluido — lo que corrige la primera redacción de este hallazgo.** Aquí se escribió que *"`ingest` sí sale ordenado"* y **era falso**: señala su fallo **lanzando** `IngestionIncomplete` (`IngestCommand.swift:98`), así que salía con `133` como los demás. El *qué* estaba bien —distinto de cero, que es lo que `D-86` le exige a *"la única señal que ve el cron"*— y el *cómo* no. **Y la causa es una sola línea del punto de entrada**: `Run/main.swift` reportaba y hacía **`throw`**, y un `throw` desde el nivel superior de un ejecutable **es** un `fatalError`. Al ser una línea en un sitio, dejó de ser mini-fase y se arregló en el acto. Lo que ya estaba bien y no cambia: el mensaje llega, el motivo verdadero llega, y **la base no se toca** —`club_atleti` intacto, 9 tablas y 480 partidos tras un `--revert` rechazado— | Antes: `migrate-tenants --revert` → aviso, `$? = 133`, mensaje **tres veces**; y con un tenant saboteado, `migrate-tenants -t a5w` → *"No se pudo migrar el club 'a5w' … 42P07 … relation «matches» already exists"*, también `133` y por triplicado | **Arreglado** — `exit(1)` en vez de `throw`, y el reporte partido en **dos** ámbitos porque **Vapor ya reporta lo que sale de `execute()`** (medido: el `report` propio era el duplicado) y **no** cubre un fallo de `configure(_:)`, que sin él saldría en silencio — peor que duplicado. **Cuatro salidas verificadas a mano**: guarda → `1` y **un** mensaje · fallo atribuido → `1` y **un** mensaje · camino bueno → **`0`** · `serve` sigue devolviendo `200` |
| **H-40** | **A-6** | **S2** · **antes de F10** — *el hallazgo del bloque* | **Los dos ficheros de *handlers* afirman que `ProblemMiddleware` no los alcanza, y es falso — medido.** `ClubHandler.swift:133-137` dice *"el transporte generado atrapa cualquier error que salga de este método y lo convierte en **500** antes de que ningún middleware de Vapor lo vea: `ProblemMiddleware` está *fuera* del transporte y solo alcanza lo que ocurre antes de entrar"*, y `IngestionHandler.swift:24-27` lo repite. **El propio `ProblemMiddleware` lo desmiente en su código**: el `case let server as ServerError` de las líneas 232-239 existe justamente para esto y su comentario lo dice con todas las letras —*"si dentro hay un error nuestro, manda el nuestro; sin esto, un `DomainError` que escapara de un handler pasaría de 422 a 500 solo por venir envuelto"*—. La mitad **verdadera** de la afirmación, que conviene no perder al corregirla, es la otra: un error **devuelto** como caso del `Output` generado sí tiene que estar declarado en el *spec*.<br><br>**Por qué importa y por qué es el hallazgo del bloque.** Es la premisa de la que cuelgan los cuatro `do/catch` locales de los *handlers* —`updateClub` atrapa `DomainError.invalidValue`, `listIngestionRuns` y `triggerIngestion` enumeran casos de `ApplicationError` uno a uno— y es la premisa que **F10 va a leer** al abrir estos ficheros para escribir `/federation-link` y `/preview`. Creyéndola, F10 escribe la traducción **otra vez en cada handler nuevo**, que es literalmente la rama *"además, cada handler"* con la que §5 define el **S1** de este bloque. Y las listas de `catch` por caso **no son exhaustivas por construcción** —`catch ApplicationError.competitionNotFound` no obliga al compilador a nada—, así que son la forma que el proyecto ya decidió no usar cuando escribió el `switch` exhaustivo del middleware | `provision-tenant a6seam` · `delete from club_a6seam.clubs` (`GetClub.execute` lanza `ApplicationError.tenantNotProvisioned` y `getClub` **no tiene `do/catch`**) · `curl -s -i http://a6seam.localhost:8080/v1/club` → **`500`** con `{"code":"TENANT_NOT_PROVISIONED","detail":"El schema del club 'a6seam' no tiene datos.","title":"Club sin aprovisionar"}` — el título y el `detail` son **palabra por palabra** los de `ProblemMiddleware.swift:119-122`, así que el error escapó del handler y lo tradujo el middleware | **Arreglado** — los dos comentarios dicen ahora la regla verdadera, que es más útil que la falsa: **lo que el contrato declara se devuelve; el resto se lanza y el middleware es la red**, con el corolario explícito de que *"no hace falta un `catch` por caso"*. Y lo que lo sostiene ya no es un comentario: `ErrorBoundaryTests`, nivel 4, **verde desde el principio** (el comportamiento siempre fue el bueno) y sostenido por **una mutación cazada** — anular el desenvuelto del `ServerError` lo pone rojo |
| **H-41** | **A-6** | **S2** · **F10** | **El *spec* promete «rol elevado» en `POST /v1/ingestion-runs` y en el código no hay dónde comprobarlo — y el único campo que podría decirlo está escrito y no lo lee nadie.** El contrato es explícito (`openapi.yaml`, descripción de `triggerIngestion`): *"Requiere **rol elevado** (§7.3): es una operación de integración, no dato del día a día"*, y declara su `403`. Pero el camino entero no tiene ancla: `ClubHandler.currentActor()` construye `ActorContext(clubSlug:)` **sin pasar `isSystem`**, o sea `false` por el valor por omisión, y `IngestClubCalendars.execute` no tiene el `TODO(§7)` que sí tiene `UpdateClub.swift:21`. **Y `isSystem` no se lee en ningún sitio**: se escribe `true` en los dos comandos y en 26 tests, `false` implícito en HTTP, y no hay un solo `switch`, `if` ni `guard` sobre él en `Sources/`.<br><br>**La contradicción de fondo, que es lo que F10 hereda.** §7.3 reparte `Team`, `OpponentClub`, `Round` y `Match` a *"la ingesta (actor de sistema)"*, y este `POST` deja que un **usuario** dispare precisamente esas escrituras (`D-88`). Así que el actor que llega al caso de uso es un usuario ejerciendo una escritura de sistema, y hoy no hay forma de expresarlo: ni el actor lo dice ni el caso de uso lo pregunta. **F10 es el mismo caso un nivel más allá** — `POST /teams/{id}/federation-link` encola una ingesta (`D-67`)—, así que decidirlo aquí lo aprovechan los dos. Lo que falta no es la política: es el sitio donde se llama | `grep -rn "isSystem" Sources/` → 3 aciertos, todos **escrituras** (`ActorContext.swift:26,28,30` la declaración; `SeedCompetitionCommand.swift:97` e `IngestCommand.swift:125` la ponen a `true`); ninguna lectura. `grep -rn "TODO(§7)" Sources/` → **un solo acierto**, `UpdateClub.swift:21`. Contrastar con `openapi.yaml` (`triggerIngestion`, *"Requiere **rol elevado** (§7.3)"* + `'403': { $ref: '#/components/responses/Forbidden' }`) | **Anclado; la decisión va a F10.** `IngestClubCalendars.execute` tiene ya su `TODO(§7)` —el segundo del proyecto— y dice **por qué no es el mismo caso** que el de `UpdateClub`: los dos llamantes traen actores de naturaleza distinta y lo que se escribe detrás es de la ingesta. Lo que **no** se decide aquí, por la regla 6 de §3: si la forma es un verbo propio del disparador o que el actor pueda decir *"actúo por cuenta de la ingesta"*. Es diseño, y F10 estrena el mismo patrón un nivel más allá |
| **H-42** | **A-6** | **S2** · **con la auth, antes de F10** | **La guarda de §6.1 —*"si no coinciden, se rechaza"*— existe, funciona, está mapeada a 403 … y no puede dispararse: compara un valor consigo mismo.** §6.1 y el propio `TenantResolutionMiddleware.swift:10-11` dicen dónde vive —*"**este middleware** es el sitio donde se compara y se rechaza la discrepancia — **no otro**"*— y el código la tiene **en otro sitio**: `FluentTenantUnitOfWork.swift:50-58`, en el adaptador de **Persistencia**, que lanza `TenancyError.tenantMismatch` y `ProblemMiddleware:201-206` traduce a **403**. La forma está bien y el 403 es el correcto; el problema es que en el camino HTTP **el actor se deriva del ambiente**: `currentActor()` hace `ActorContext(clubSlug: try Slug(TenantContext.current.slug))` y luego la guarda pregunta `ambient.slug == actor.clubSlug.value`. `Slug.init(_:)` **valida y no normaliza** (`Slug.swift:23`, `self.value = value`), así que la igualdad es una **tautología**. Y en el camino del comando `TenantContext.current` es `nil`, con lo que la guarda no se ejecuta. **Conclusión: `tenantMismatch` es inalcanzable en producción hoy, por las dos vías.**<br><br>**Por qué no es cosmético.** Es la regla que §6.1 llama la que *"evita la clase entera de confusiones «el token dice un club y la URL dice otro»"*, o sea la costura de aislamiento entre clubes, y **no tiene ni un test**. Cuando aterrice JWKS, `currentActor()` pasará a construir el actor **desde el *claim*** y la comparación dejará de ser tautológica de golpe: es una línea, y hoy no hay nada en verde que diga si el 403 sale bien. Además hay que decidir cuál de los dos hogares es el bueno — el middleware es una **puerta** (corta antes de tocar datos y cubre cualquier endpoint futuro); el `UnitOfWork` es un **cinturón** que solo se abrocha si alguien abre un ámbito de tenant. Los dos son defendibles; lo que no lo es es que los documentos nombren el que no lo hace | `grep -rn "tenantMismatch\|TENANT_MISMATCH" Sources/ Tests/` → **4 aciertos y ninguno en `Tests/`**: la declaración (`Tenant.swift:19`), el único `throw` (`FluentTenantUnitOfWork.swift:54`) y su traducción a 403 (`ProblemMiddleware.swift:201,203`). La tautología se lee en dos saltos: `ClubHandler.swift:80` construye el actor desde `TenantContext.current`, `FluentTenantUnitOfWork.swift:53` lo compara con `TenantContext.current` | **Arreglado lo que era arreglable hoy, y la discrepancia resuelta a favor de los dos.** Los documentos decían *"este middleware y no otro"*: ahora dicen **puerta y cinturón** — la puerta va en el middleware cuando exista el *claim* (corta antes de tocar datos y cubre cualquier endpoint), el cinturón se queda en el `UnitOfWork` (solo se abrocha si alguien abre un ámbito). Ninguno de los dos sobra, y el que estaba sin escribir era el que existe. **Y ya tiene test**: `ErrorBoundaryTests`, con el actor construido a mano —que es lo que lo hace escribible **antes** de JWKS y no después—, con su mutación cazada (la guarda que nunca rechaza). Lo que queda es una línea de `currentActor()` el día que el actor salga del *claim*, y entonces el test deja de ser hipotético sin tocarlo |
| **H-43** | **A-6** | **S3** — *cierra la mitad de diagnóstico de H-34 y es la de H-15* | **El error no clasificado pierde su motivo en las **dos** salidas, y el arreglo es una línea con la función que ya existe.** `ProblemMiddleware.translate` tiene el `switch` exhaustivo sobre `DomainError`, `ApplicationError` y `TenancyError`, y **todo lo demás** cae en dos ramas que se quedan con la descripción corta: el `ServerError` genérico usa `server.causeDescription` —que es el literal `"User handler threw an error."`— y el `default` usa `String(describing: error)` (línea 251). Con un `PSQLError` eso es exactamente el defecto que F6 ya arregló en `IngestionRun`: PostgresNIO devuelve *"Generic description to prevent accidental leakage of sensitive data. For debugging details, use `String(reflecting: error)`"*. **Y lo nuevo que mide este bloque: el log del servidor pierde el motivo igual que el cuerpo** —el `request.logger.report(error:)` de la línea 52 imprime la descripción corta—, así que en producción, donde el `detail` de los 5xx se calla a propósito (`Problem.response`, `exposesInternalDetail`), **no queda ninguna salida con el motivo verdadero**.<br><br>**Por qué es S3 y no más.** `diagnosticText(for:)` —`String(reflecting:)`, con su porqué escrito en `ErrorDiagnostics.swift`— vive en `Application`, `HTTPAdapter` ya importa `Application` y `IngestionHandler.swift:261` ya lo usa. Un fichero, y el riesgo de filtración lo acota el mecanismo que ya está puesto: fuera de desarrollo el `detail` de un 5xx no viaja. **Es la mitad de H-34 que quedó abierta y el sitio donde H-15 aterriza** cuando `FederationError` empiece a cruzar la frontera dentro de la petición (el `/preview` de F10): hoy los cuatro casos caerían en este mismo `default` | Mismo montaje que H-40, con la tabla fuera: `drop table club_a6seam.clubs` · `curl -s -i http://a6seam.localhost:8080/v1/club` → **`500`** con `{"code":"INTERNAL","detail":"User handler threw an error.","title":"Error interno"}`. Y en el log del servidor, la misma petición: `[ WARNING ] Server error - cause description: 'User handler threw an error.', underlying error: PSQLError – Generic description to prevent accidental leakage of sensitive data. For debugging details, use ``String(reflecting: error)``…`. Contrastar con el caso clasificado del mismo montaje (H-40), que sí dice el club y la causa | **Arreglado** (un fichero, cuatro sitios): las dos ramas que se quedaban con la descripción corta usan `diagnosticText(for:)`, y **la rama del `ServerError` desenvuelve primero** —`causeDescription` se conserva en los **4xx**, donde sí dice algo útil (*"Missing required query parameter named: …"*)—. Ahora el cuerpo lleva `sqlState: 42P01, message: relation "clubs" does not exist` con su SQL. **Y el log también, que era la mitad que importa en producción** porque allí el `detail` de un 5xx se calla. Dos tests y **dos mutaciones cazadas**. *Y una lección dentro del arreglo*: la primera versión **pasó el test del cuerpo y dejó el log igual de enmascarado** —reflejaba el envoltorio, y el envoltorio describe a su contenido con `String(describing:)`—; lo delató el propio log de los tests, dos líneas seguidas con el mismo *"Generic description…"*. **Dos salidas son dos caminos: hacían falta dos aserciones** |
| **H-44** | **A-6** | **S4** — la respuesta a la costura 5 | **Hoy existe el enumerador de tenants que §9.10 dice que no hay, y lo tapa una restricción que ya está puesta por otro motivo.** Sin auth, cualquiera distingue un club existente de uno que no existe con una petición y sin credenciales: **404** `UNKNOWN_TENANT` frente a **200**/**500**. Es consecuencia de la deuda declarada de F0, no un defecto nuevo, y se anota porque §9.10 cerró la cuestión *"cómo llega un usuario a su subdominio"* justamente con el argumento de que así **"desaparece la única superficie no autenticada que habría enumerado tenants"** — mientras la auth no esté, esa superficie **es toda la API**.<br><br>**Y la buena noticia, que es el hallazgo: el orden que lo arregla no hay que elegirlo, ya está forzado.** `TenantResolutionMiddleware` tiene que ir **el último** de la cadena por el problema conocido entre `@TaskLocal` y la implementación interna de Vapor (`Configure.swift:82-84`), así que el middleware de auth solo puede colgarse **por fuera** — y por fuera significa que corre **antes**: un no autenticado recibe **401** sin que la resolución de tenant llegue a consultar `public.tenants`. La restricción que parecía una limitación de arnés es lo que hace segura esta costura, y de paso deja disponible el *claim* para quien tenga que compararlo (H-42). No hay nada que hacer hoy; queda escrito para que nadie reordene la cadena sin saber qué más se lleva | `curl -s -i http://noexiste.localhost:8080/v1/club` → **`404`** `UNKNOWN_TENANT`, `detail: "noexiste"` · `curl -s -i http://atleti.localhost:8080/v1/club` → **`200`** · `curl -s -i http://localhost:8080/v1/club` (sin club en el `Host`) → **`400`** `TENANT_NOT_RESOLVED`. Los tres sin una sola credencial | **Nota** — no se arregla, y queda **anclado donde se rompería**: `Configure.swift` dice ahora que el orden forzado de la cadena cierra el oráculo, *"al reordenar esta cadena, saber que se lleva las dos cosas por delante y no solo el `@TaskLocal`"* |
| **H-45** | **A-7** | **S2** · **F7** — *el hallazgo del bloque* | **La mitad *"se para"* de `D-86` enmendada no la guarda nada: la sonda del ámbito 1 no la ejecuta ni un test, y el freno que cuelga de ella cruza dos *targets* sin nadie que fije el par.** `D-86` enmendada tiene **dos** frenos y A-3 verificó por mutación **uno**. El que sí está guardado es el del bucle por competición dentro de un club (`IngestClubCalendars.swift:125`, `report.abortedByInfrastructure`, cazado por `aDatabaseOutageStopsTheTraversal`). El que no es **el del recorrido de clubes**: `IngestClubCalendars.swift:68-73` pregunta a la base si sigue ahí cuando falla **el plan** —el ámbito 1— y, si no, lanza `ApplicationError.databaseUnavailable`; `IngestCommand.swift:148` hace `if case ApplicationError.databaseUnavailable = error { break }`. **Ese par es lo único que hace que los clubes que faltan no se recorran con la base caída**, que es lo que `AGENTS.md` afirma —*"se para también el recorrido de los clubes que faltan, porque el pool y el Postgres son uno solo (§6.4)"*— y lo que `D-86` declara inseguro si no ocurre: continuar sin dejar constancia.<br><br>**Y el hueco es de un solo lado, que es lo que lo hace fácil de pasar por alto.** La mitad *"se continúa"* sí está guardada: hacer el `break` incondicional lo caza `un club sin adaptador no detiene el recorrido de los demás (D-86, D-17)`. La mitad *"se para"* no tiene test **en ninguno de los dos extremos**: `TenantTraversalTests` tiene siete tests del recorrido multi-club y **ninguno con la base caída**, y `databaseUnavailable` no aparece en `Tests/` ni una vez. Nada que ver con descuido de A-3: su ronda instaló **dos** sitios de sonda y mutó el que su test alcanzaba | Tres mutaciones, y el árbol restaurado con `git checkout` después de cada una. **M1** · `IngestClubCalendars.swift:71`, `databaseUnavailable` → `unknownSeason(id:)` → **`REQUIRE_DB=1 swift test` sigue en verde, 297/297**. **M1c** · se borra el bloque entero `if await !databaseResponds(actor:) { throw … }` de las líneas 68-73, dejando `throw error` en crudo → **sigue en verde**. **M1b** *(el reverso, para enseñar que el hueco es de un lado)* · `IngestCommand.swift:148` → `break` incondicional → **cazada**, por `un club sin adaptador no detiene el recorrido de los demás` | **Arreglado en su mitad alcanzable, y la otra queda acotada y elevada.** El **lado que lanza** tiene test (`si la base no está al leer el plan, se dice con su nombre`, nivel 2, con `SwitchableUnitOfWork` bajado **antes** del plan y aserción **sobre el caso**): mata M1 y M1c. El **lado que empareja** —`IngestCommand.swift:148`— **no se puede probar sin tocar producción**: la unidad de trabajo se construye dentro de `ingest` desde `app.db(.control)` y no es inyectable, y provocar el fallo de verdad exige parar Postgres, que A-3 ya descartó hacer desde un test. **Dispara la válvula de §3 regla 2** → elevado, con la salida ya identificada: extraer *"¿este resultado detiene el recorrido?"* como regla pura, que es **el hermano que el proyecto ya tiene** (`incomplete(outcomes)` está separado *"porque es la regla, no el print"*) |
| **H-46** | **A-7** | **S2** · **F10** | **Lo que las aserciones de error fijan es el tipo, no el caso — y cada caso de `ApplicationError` es un código HTTP distinto, razonado uno a uno.** En `Tests/` hay **48** aserciones `#expect(throws: X.self)` frente a **8** sobre un caso concreto. Para un *Value Object* con un solo modo de fallo eso es correcto y no se toca (`Slug`, `MatchResult`, `SeasonLabel`: el montaje del test fija el caso). Donde sí importa es en `ApplicationError`, porque `ProblemMiddleware` traduce **cada caso a un status deliberadamente distinto del de su vecino**, y cada uno lleva su párrafo explicando por qué: `competitionNotFound` → **404**; `seasonNotFound` → **500** *"al revés que la de arriba"*; `unknownSeason` → **404** *"al revés que `seasonNotFound`"*; `databaseUnavailable` → **503 y no 500** (H-23); `runNotRecorded` → **500**, y su `detail` existe para que quien lo reciba **no repita una ingesta que sí se escribió**. Intercambiar esos casos deja la batería en verde: un 404 pasa a 500, un 500 a 404, y el `RUN_NOT_RECORDED` que dice *"los datos están escritos"* pasa a un `DATABASE_UNAVAILABLE` que invita justo al reintento que el comentario prohíbe.<br><br>**Por qué hoy no llega a un cliente, y por qué es S2 y no S3.** Los casos que sobreviven son los que lanza **la pasada**, que no cruza HTTP (§2.3-b): el `202` responde antes, y el `GET` tiene su propio `competitionNotFound` en `IngestionHandler.swift:72`, que es el que sostiene el 404 del test. El propio middleware lo anticipa en su comentario de las líneas 145-150: *"los levanta la pasada de ingesta, que no pasa por HTTP … y **F10 sí los va a hacer cruzar la frontera**: el enganche de `D-67` encola una ingesta y su `/preview` la ejecuta en línea"*. **El razonamiento está escrito y no lo hace cumplir nada** — y lo que lo confirma es qué mutación sí se caza: `unknownSeason` es el único de los cinco con un test HTTP detrás, y es el único cazado | Tres mutaciones más, mismo protocolo. **M3** · `IngestCalendar.swift:104`, `runNotRecorded` → `databaseUnavailable` → **verde**. **M4** · `IngestCalendar.swift:126` y `:146`, `competitionNotFound` → `seasonNotFound` → **verde**. **M5** · `IngestCalendar.swift:130` y `:155`, `seasonNotFound` → `competitionNotFound` → **verde**. **M2** *(el contraste)* · `IngestClubCalendars.swift:228`, `unknownSeason` → `databaseUnavailable` → **cazada**, por `una temporada que no existe no cae a la vigente: 404 (D-84)`. Y la medida de fondo: de los **14** códigos `Problem` que el middleware puede emitir, **3** los afirma algún test (`INTERNAL`, `INVALID_VALUE`, `TENANT_NOT_PROVISIONED`) | **Arreglado** — cinco casos fijados **por caso y no por tipo**, con tres tests nuevos y dos aserciones fortalecidas. Mata M3, M4, M5 y, de paso, M7 y M8, que no se habían probado: las dos guardas del plan solo las cazaba el 404 de nivel 4 (`unknownSeason`) o nada en absoluto (`competitionNotFound`). Lo que **no** se ha tocado es la superficie HTTP: los códigos `Problem` siguen afirmados 3 de 14, y eso es de F10, que es quien los hará cruzar |
| **H-47** | **A-7** | **S3** *documental, y es la respuesta del bloque a la categoría que hereda* | **El corolario de H-38 vale también al revés, y el propio instrumento de este libro lo demuestra: lo que `grep` no encuentra no prueba que falte un test.** A-6 le dejó escrito a A-7 que *"lo que `grep` demuestra es que algo está, no que haga lo que dice"*. La otra mitad es peor, porque produce hallazgos falsos en vez de tranquilidad falsa: el cruce por **nombre de caso** —el instrumento con el que A-0 verificó las citas `D-nn` y el primero que usó este bloque— señaló **seis** casos de error *"sin test"*, y la mutación desmintió **tres**. El ejemplar es `DomainError.federationSourceMismatch`, la guarda del nombre de `D-84`: **cero** apariciones en `Tests/`, ningún test llama a `requireSameSource`, y las dos únicas menciones son comentarios, uno de ellos describiendo el caso en que la guarda **calla**. Anular la guarda la cazan **dos** tests. Está probada por su efecto —la pasada se para y no escribe— sin nombrar nunca el caso, que es exactamente como debe ser en el nivel 2.<br><br>**La regla que se añade, y que es para los bloques y no para el código:** un cruce por nombre sirve para **levantar la sospecha**, nunca para cerrarla. Lo que cierra la pregunta *"¿lo prueba algo?"* es **romperlo y mirar**. Con el corolario de A-6 forman el par: `grep` no demuestra ni que algo funcione ni que algo falte; la mutación demuestra las dos | `grep -rn federationSourceMismatch Tests/` → **0**. `grep -rn requireSameSource Tests/` → **2**, las dos en comentarios. Mutación **M0** · `Competition.swift:276`, `requireSameSource` devuelve sin opinar → **cazada por dos**: `una coordenada que apunta a otra competición para la pasada (D-72, D-84)` y `la pasada que falla también deja registro, con su motivo (D-85)` | **Anotado** — no hay código que arreglar; corrige el **método** del libro, y con él la lectura que A-5 y A-6 le dejaron a este bloque |
| **H-48** | **A-7** | **S4** — la respuesta a *"¿tiene borde la categoría?"* | **La categoría *"fuera del arnés"* tiene dos bordes y no uno, y el caso que A-4 metió en ella estaba clasificado por el motivo equivocado.** A-4 le dejó a A-7 que *"`DetachedBackgroundWork` no lo toca ningún test, que es código fuera del alcance del arnés por construcción"*. Lo primero es **falso**: `TestSupport/TestEnvironment.swift:157` lo usa como ejecutor **por defecto** del nivel 4, así que lo ejecutan todos los tests que no pasan `background:`. Y el hermano que A-6 manda buscar **también existía**: `InlineBackgroundWork`, escrito justamente para que una aserción vea el resultado. Pero la conclusión de A-4 **se sostiene**, por otra razón: la única propiedad que el fichero se atribuye —*"`Task { }` y no `Task.detached`: hereda los `@TaskLocal`, y uno de ellos es el tenant"*— **no es observable**, porque `FluentTenantUnitOfWork.resolveTenant` conserva a propósito la vía sin ambiente para el job (§2.3-b) y resuelve el tenant del `actor` capturado. Las dos variantes se diferencian en **un `SELECT` redundante** sobre `public.tenants`, no en corrección — y es el mismo `SELECT` duplicado que el comentario de las líneas 41-44 dice haber quitado.<br><br>**Así que los dos bordes, en una línea cada uno, que es lo que el bloque tenía que entregar:** **(a)** la observación exige **la frontera del proceso** — el código de salida de un comando (H-39), que no se ve desde dentro del proceso que lo produce; **(b)** el programa mutado **es el mismo programa** — M6, la tercera lectura que F5 aportó. **Y H-38 no era ninguna de las dos: era *"nadie fue a mirar"*,** que es lo que el propio A-5 demostró automatizándola. La categoría es estrecha, y esa es la conclusión | Mutación **M6** · `BackgroundWork.swift:30`, `Task { }` → `Task.detached { }` → **sobrevive**, y sobrevive *con razón*: `FluentTenantUnitOfWork.swift:46-49` documenta la vía sin ambiente y `:50` la bifurca. `grep -rn BackgroundWork Sources/TestSupport/` → `TestEnvironment.swift:146,157` | **Nota** — no se arregla. Corrige el renglón que A-4 dejó en §7 y le pone borde a la categoría |
| **H-49** | **A-7** | **S3** — va con el mismo deber de despliegue que el cron | **Las dos comprobaciones que el proyecto publica fuera de `swift test` funcionan, y ninguna de las dos es reproducible sin saber algo que no está escrito.** (1) **El `lint` del *spec*** que `AGENTS.md` y el README publican **no corre en un shell no interactivo**: `node` está instalado por `nvm` (v18.15.0 y v22.17.0) y `nvm` solo carga en shell interactiva, así que `npx` no existe en el `PATH` que vería cualquier automatización. Con la ruta explícita pasa —*"Your API description is valid"*, 99 ms—, pero avisando de algo que sí es sustantivo: *"No configurations were provided — using built in recommended configuration by default"*. **No hay `redocly.yaml` ni versión fijada**, así que *"el spec es válido"* significa hoy *"válido para el conjunto de reglas por defecto de la versión que `@latest` devuelva"* — y eso cambia sin que nadie toque el *spec*. (2) **`swift build` sí gana su sitio en el *workflow***, y queda medido: añadir al `filter` una operación sin *handler* rompe el *build* con `type 'APIHandler' does not conform to protocol 'APIProtocol'`. El trinquete de `D-69` lo hace cumplir el compilador, no la disciplina | `which npx` → *not found*; `ls ~/.nvm/versions/node` → `v18.15.0`, `v22.17.0`. Con `PATH` explícito: `npx @redocly/cli lint …` → válido, con el aviso de configuración. Y `printf '    - listTeams\n' >> openapi-generator-config.yaml && swift build` → **exit 1**, con el error de conformidad (restaurado después) | **Abierto** — se cierra con el *workflow*, que es la misma decisión de despliegue que el cron (`D-87`, Plan §9) |
| **H-50** | **A-7** | **S4** | **`swift-metrics` no es una dependencia nuestra sin usar: es transitiva de Vapor, y eso abarata la propuesta en vez de ensuciarla.** A-3 le dejó apuntado a A-7 que *"`swift-metrics` está en el grafo de dependencias y no se usa"*. Las dos mitades son ciertas y falta el matiz que decide qué hacer: **no aparece en `Package.swift`**, así que no hay nada que limpiar ni que justificar — llega gratis con Vapor y está disponible. Sigue en pie el fondo del apunte de A-3, que es la señal que **no comparte suerte con la base**: un contador de pasadas fallidas y un *gauge* de *"minutos desde la última pasada con éxito"* no se leen de `ingestion_runs`, que es precisamente la tabla que no se puede escribir cuando lo que falla es Postgres (`D-86` enmendada). Queda como material de la decisión de observabilidad, que va con el cron | `grep -n "metrics\|Metrics" Package.swift` → **vacío**. `grep -rn "import Metrics\|Counter(\|Gauge(" Sources/` → **vacío** | **Nota** — insumo para el deber de despliegue, no deuda de código |
...
### Nota de cierre de A-6 · ¿están puestas las costuras, o F10 escribirá los *handlers* dos veces?

**Están puestas, y la respuesta al listón del bloque es «cinco ficheros, una línea cada uno».** Ninguno de
los cinco hallazgos es S1: **la arquitectura de §7 cabe en lo que hay sin tocar ni un *handler***. Lo que A-6
encuentra no es una costura ausente, sino tres costuras **presentes y desmentidas por lo que está escrito
encima de ellas** — que es una clase de defecto que este libro ya conoce (H-01, H-11) y que aquí tiene una
consecuencia concreta, porque el lector previsto es F10.

- **La costura mejor puesta no es una convención: es un tipo.** `TenantUnitOfWork.withRepositories(actor:)`
  **exige** el `ActorContext`, así que un caso de uso que toque datos de tenant no puede olvidarlo — no
  depende de que alguien se acuerde de la regla de `AGENTS.md`. Los cinco puntos de entrada públicos lo
  reciben (`GetClub`, `UpdateClub`, `IngestCalendar`, `IngestClubCalendars.execute` y `plannedCompetitions`),
  y `CalendarPass` no lo lleva **con razón**: es maquinaria interna que ya corre dentro de un ámbito abierto
  con el actor.
- **El tenant se decide en un solo sitio, y la traducción de actor también.** `currentActor()`
  (`ClubHandler.swift:76-81`) es el **único** traductor de ambiente a actor y **los cuatro** *handlers* lo
  llaman. Ningún repositorio lee el `Host`; el único lector del `@TaskLocal` fuera del middleware es
  `FluentTenantUnitOfWork`, y lo hace para **no repetir el `SELECT`** del middleware, con su porqué escrito.
- **El 403 de §7.5 es representable hoy, y por dos vías independientes.** El *spec* declara `401` en las 83
  operaciones y `403` en 44, y las dos operaciones de escritura del alcance entregado lo tienen — comprobado
  en el **código generado**, no en el YAML: `Operations.updateClub.Output` y
  `Operations.triggerIngestion.Output` traen su `case forbidden`. Y si el error se **lanza** en vez de
  devolverse, el `switch` exhaustivo de `ProblemMiddleware` lo obliga a decidir su HTTP antes de compilar,
  que es el mecanismo que F6-bis acaba de ejercitar con `federationSeasonMismatch`.

**El listón, contestado en número de ficheros.** Poner §7 en marcha toca, de lo que ya existe:

| # | Fichero | Qué cambia |
|---|---|---|
| 1 | `App/Configure.swift` | una línea más en la cadena de `grouped(…)`, **por fuera** de la de tenancy (H-44) |
| 2 | `HTTPAdapter/ClubHandler.swift:76-81` | `currentActor()` construye el actor **desde el *claim*** y carga `StaffMember` + asignaciones. **Una función, y los cuatro *handlers* ya la llaman** |
| 3 | `Application/ActorContext.swift` | los campos que §7.4 pide, que su propia documentación ya anticipa |
| 4 | `Application/UpdateClub.swift:21` | el `TODO(§7)` pasa a ser la llamada a la política |
| 5 | `HTTPAdapter/ProblemMiddleware.swift` | el `case` del error de autorización — **y no compila hasta ponerlo** |

Más lo que es **implementar** §7 y no costura: el middleware JWKS, el puerto de política con su
implementación, y las tablas de §7.3 con su migración (que va por `D-90`). §8 lo deja fuera a propósito.

**La lección transferible, y es la tercera vez que el libro la escribe con otra cara.** A-0 la encontró en
documentos (H-01), A-5 en el eje del tiempo (H-31), y aquí aparece en **comentarios de código**: el `ServerError`
de `ProblemMiddleware:232-239` se escribió en F6 —lo pidió un test— y **nadie volvió a los dos comentarios que
afirmaban lo contrario**. Un comentario no tiene batería que lo tumbe, así que sobrevive a la línea que lo
desmiente y la contradice desde el fichero de al lado. La regla que se añade: **cuando un arreglo desmiente una
premisa, hay que ir a buscar dónde estaba escrita** — y en este proyecto el sitio más probable no es el LLD,
que se revisa, sino un `///` de tres párrafos que se escribió una vez y se leyó siempre.

**Y una que es del método del bloque, no de su resultado.** Las tres costuras buenas se comprobaron con
`grep` y con el código generado; las tres afirmaciones falsas **solo se cayeron ejecutando el servidor** —dos
`curl` contra un tenant saboteado a propósito—. Es el mismo patrón que A-3 y A-5 dejaron apuntado y que A-7
tiene que leer como categoría (H-38): *lo que `grep` demuestra es que algo está; que haga lo que dice, no*.

### Lo que A-6 entrega: el inventario de costuras

| # | Costura de §5 | Veredicto | Hallazgo |
|---|---|---|---|
| 1 | Todos los casos de uso reciben `ActorContext` | ✅ **y lo fuerza el puerto**, no la disciplina | — |
| 2 | Un solo sitio decide el tenant | ✅ un traductor (`currentActor()`), cuatro llamantes, cero lecturas del `Host` fuera del middleware | — |
| 3 | La comprobación de ámbito tiene dónde caer | ⚠️ **una de dos**: `UpdateClub` sí; el disparador de ingesta, que el *spec* promete con «rol elevado», no | **H-41** |
| 4 | 403, no 404, es representable | ✅ por dos vías: el `case forbidden` generado en las dos operaciones de escritura, y el `switch` exhaustivo | — |
| 5 | El 403 vs 404 no filtra existencia de tenants | ✅ **por el orden forzado** de la cadena de middlewares; hoy sin auth sí filtra, y es la deuda de F0 | **H-44** |
| 6 | *(heredada de A-5)* Quién traduce el error a diagnóstico | ⚠️ **un** sitio, y pierde el motivo en las dos salidas. Una línea | **H-43**, con **H-15** detrás |
| — | *(lo que el bloque no venía a buscar)* | ❌ los *handlers* documentan lo contrario de lo que el middleware hace | **H-40** |
| — | *(idem)* | ❌ la guarda de §6.1 compara un valor consigo mismo y no tiene test | **H-42** |

**Lo que este bloque necesitó y no estaba en su «Leer antes»**, para quien lo relea: **el *spec* y el código
generado**. La costura 4 no se puede contestar con el YAML —que declara `403` en 44 operaciones— sino con
`Operations.…Output`, y eso exige un `swift build` primero, porque `APIContract` **no existe en disco** hasta
que el plugin corre (`D-69`). El fichero está en
`.build/plugins/outputs/backend/APIContract/destination/OpenAPIGenerator/GeneratedSources/Types.swift`. Y
**hizo falta el servidor de verdad**, al contrario de lo que el bloque suponía al presupuestarse: media sesión
de montaje —`docker compose up -d`, `provision-tenant`, y **vaciar** la tabla `clubs` de un tenant de usar y
tirar para que un `ApplicationError` escape del *handler*—, pero **no** hace falta ni red ni `seed-competition`,
que es lo que encarecía a A-4. Dos avisos: el club de trabajo **no** se toca (comprobado al cerrar:
`club_atleti` con sus 480 partidos), y la tabla de hallazgos de §6 tenía una **línea en blanco** entre H-38 y
H-39 que la partía en dos y dejaba a H-39 fuera de la tabla al renderizar — corregida en el acto por la
excepción documental de §2.
...
> **Y al cerrar A-7 aparece una casilla que esta tabla no tiene, así que se dice en vez de forzarla.** Los dos
> S2 que quedan abiertos —**H-07** (el CI que no existe) y su vecino **H-49** (el `lint` no reproducible)— no
> se arreglan en **ninguna fase**: se arreglan **montando el *workflow***, que es la misma decisión de
> despliegue que el cron de `D-87` y que Plan §9 ya lleva apuntada como deber del desarrollador. Por la letra
> de la tabla, un S2 sin fase *"cuenta como S1"* y **la puerta estaría cerrada**; por su espíritu —*"una deuda
> sin fecha es una deuda olvidada"*— no lo está, porque la fecha existe y no es un número de fase. Así que la
> casilla que falta es **«deber de despliegue»**, con estas dos dentro y el cron al lado, y **quién decide si
> F7 arranca antes o después de montarlo es el desarrollador**, no la auditoría. Lo que A-7 sí deja cerrado es
> que **no hay nada que programar**: el *workflow* está propuesto, los cuatro pasos están medidos, y las tres
> condiciones que debe exigir están escritas y corregidas.
>
> **Y la última fila de la tabla queda contestada:** el comprobador de cifras **no hay que escribirlo**.
> `swift test --xunit-output` ya emite el recuento y los omitidos con su motivo (H-07), así que *"el recuento
> al día"* pasa de ser disciplina a ser una consulta al XML.

**Estado de la puerta al cerrar la ronda de A-7** — los ocho bloques están cerrados y esto es lo que queda:

| Condición | Cómo está |
|---|---|
| **Cero S1 abiertos** | ✅ ninguno, en ningún bloque |
| **Cada S2 con fase** | ⚠️ **uno sin asignar**: la mitad elevada de **H-45** (el `break` de `IngestCommand:148`). La válvula de la regla 2 disparó, así que **necesita entrada propia en el Plan de desarrollo** —como la tuvo F6-bis— y eso lo decide el desarrollador, no la auditoría. Lo demás sí: H-15 y H-46 → **F10**, H-21 → la fase que abra la corrección de `OpponentClub`, H-40 y H-42 → **antes de F10** |
| **S3 documentales, corregidos** | ✅ incluido el recuento: **300 tests** en `AGENTS.md` y en las dos apariciones del `README` |
| **Batería en verde y recuento al día** | ✅ `REQUIRE_DB=1 swift test` → 300, 0 fallos, 1 omitido (el canario), 5,70 s y 2,76 s |

Y **dos deberes que no son de ninguna fase** y que van juntos porque son la misma decisión de despliegue:
**montar el *workflow*** (H-07, H-49) y **montar el cron** (`D-87`, Plan §9).
...
| **A-5** · Las migraciones | ● **cerrado**. **Cero S1**: los dos caminos dan un solo `md5` y el `revert` deshace en bloque — y desde ahora eso lo guarda un test y no una sesión de auditoría (H-38). El riesgo estaba en el otro eje —**editar una migración ya aplicada**— y es `D-90`. **§9.3 del LLD queda decidida**: se para, se descarta paralelizar, y quedan dos deberes de operación en el Plan | 2026-09-12 / 09-13 | H-30 · H-31 · H-32 · H-33 · H-34 · H-35 · H-36 · H-37 · H-38 · **H-39** (de la ronda, no del bloque, y arreglado en ella: *"todos los comandos"* y *"una línea"* eran la misma frase) |
| **A-6** · Las costuras de §7 | ● **cerrado** — **cero S1**: las costuras están y poner §7 en marcha toca **cinco ficheros, una línea cada uno**, sin tocar ni un *handler*. Lo que fallaba no eran las costuras sino **lo escrito encima**, y ya está corregido (`d866e84` la auditoría, ``111fc56`` la ronda): los dos comentarios que negaban al middleware, el *"y no otro"* que ahora es **puerta y cinturón**, el `TODO(§7)` que le faltaba al disparador y el aviso del orden de la cadena. **Y el diagnóstico enmascarado se arregló de verdad**: cuerpo **y** log, que cierra la mitad abierta de H-34 y le deja el aterrizaje hecho a H-15. La frontera de error tiene test por primera vez | 2026-09-13 / 09-14 | H-40 · H-41 · H-42 · H-43 · H-44 (+ **H-34** cerrado en su mitad de diagnóstico y **H-15** acotado a F10) |
| **A-7** · El arnés | ● **cerrado** — **cero S1**, y la respuesta a *"qué significa verde"*: hoy significa *"$? == 0"* y nada más, porque **el texto de la salida no distingue una batería que corrió de una que se omitió** (H-07 reproducido con 297 tests: mismos seis renglones, mismo recuento, solo cambia la duración). El remedio no es leer mejor el texto: es `--xunit-output`, que trae los recuentos y el motivo del omitido en XML. **Los dos hallazgos del bloque salieron de mutar, no de leer**: la mitad *"se para"* de `D-86` enmendada no la guarda nada (**H-45**) y las aserciones de error fijan el tipo y no el caso, cuando cada caso es un HTTP razonado aparte (**H-46**). Y las tres lecturas heredadas quedan contestadas: el corolario de `grep` **vale también al revés** (**H-47**), la categoría *"fuera del arnés"* tiene **dos bordes** y el caso de A-4 estaba clasificado por el motivo equivocado (**H-48**), y `swift-metrics` es transitiva, no deuda (**H-50**). **Ronda hecha** (`REQUIRE_DB=1 swift test` → **300**, 0 fallos, 1 omitido): H-46 cerrado entero y H-45 por el lado que lanza, con **siete mutaciones, siete cazadas**. Lo que queda es **un solo deber elevado** —el `break` de `IngestCommand:148`, que no se prueba sin una API nueva— y **el *workflow***, que no es de ninguna fase | 2026-09-14 | H-07 (desde A-0) · la nota de cierre de A-2 · las **tres condiciones** que le deja la ronda de A-2: `$? == 0`, `skipped == 0` y **no leer el resumen de texto**, que no existe · y de A-4: **`DetachedBackgroundWork` no lo toca ningún test**, que es código fuera del alcance del arnés por construcción · y de A-5: **`H-38`**, que es la tercera vez que el libro encuentra *"la garantía depende de repetirlo a mano"* (H-17, H-26, H-38) — conviene que A-7 lo lea como **patrón**, no como tres hallazgos · y de A-6, **los dos huecos ya tapados por su ronda y una vuelta de tuerca al patrón**: los tests que faltaban —la traducción de lo que se escapa de un *handler* (H-40) y `tenantMismatch` (H-42)— **existen** (`ErrorBoundaryTests`), así que lo que le queda a A-7 no es la deuda sino **la lectura**: (a) el patrón de H-38 aparece aquí **en su forma más barata** —las tres costuras buenas las demostró `grep`, y las tres afirmaciones falsas solo se cayeron **ejecutando el servidor**: *lo que `grep` demuestra es que algo está, no que haga lo que dice*—; y (b) **la corrección al propio corolario de H-38**, que es la más útil: la mutación del log *"estaba fuera del arnés por construcción"* y **no lo estaba** — el proyecto ya había resuelto el caso hermano (`APIHandler` inyecta su `Logger` por H-27, y `CapturingLogHandler` ya existía). Antes de clasificar algo como no-automatizable, buscar el hermano |
...
$ Run migrate-tenants -t a5w          # con una tabla `matches` saboteada
→ migrando a5w (schema club_a5w)
[ WARNING ] No se pudo migrar el club 'a5w' (schema club_a5w); el recorrido se
            para aquí: PSQLError(… 42P07 … relation "matches" already exists)
```

**Las dos decían lo que tenían que decir, y las dos terminaban mal: código `133` y el mensaje por
triplicado.** De ahí salió **H-39**, que es de la ronda y no del bloque.

**Se anotó como mini-fase y acabó arreglándose en el acto, porque al buscar el sitio el alcance se
desplomó.** La primera lectura decía *"el punto de entrada de todos los comandos, válvula de la regla 2"*, y
es cierto que toca a los cinco — pero el cambio es **una línea**: `Run/main.swift` reportaba el error y hacía
`throw`, y un `throw` desde el nivel superior de un ejecutable **es** un `fatalError`. Cambiarlo por
`exit(1)` arregla los cinco comandos a la vez, y de paso destapó dos cosas que solo se ven ejecutando:

- **El duplicado era nuestro.** Vapor **ya reporta** el error que sale de `execute()`; el
  `app.logger.report(error:)` del punto de entrada lo imprimía otra vez. Medido quitándolo: el mensaje pasa a
  salir **una** vez. Pero quitarlo del todo dejaría **en silencio** un fallo de `configure(_:)`, que Vapor no
  cubre — así que son **dos ámbitos**, uno por cada dueño del reporte. *"Duplicado"* tenía arreglo; *"en
  silencio"* no lo habría tenido.
- **La primera redacción de H-39 se equivocaba en el contraste.** Decía que `ingest` *"sí sale ordenado"*
  porque deriva su fallo de `hasFailures`; lo que hace es **lanzar** `IngestionIncomplete`, así que salía con
  `133` como los demás. Derivar **si** falló y decidir **cómo se cuenta** son dos cosas, y estaban mezcladas.
...
> ceremonia: es la clase de fallo que los niveles 2 y 3 no pueden ver por construcción. **Y el corolario, de
> esta vez:** antes de aplazar algo por *"toca muchos sitios"*, ir a mirar **cuántas líneas** son. Aquí
> *"todos los comandos"* y *"una línea"* eran la misma frase.

**Cómo quedó, verificado contra el comando** —y ésta es la comprobación que sustituye al test que no existe,
porque el código de salida de un proceso está fuera del arnés por construcción (nota para A-7):

```
migrate-tenants --revert          → código 1 · el aviso, UNA vez · base intacta
migrate-tenants -t a5w (saboteado) → código 1 · el motivo con su club, UNA vez
migrate-tenants                    → código 0 · "1 tenant(s) procesados."
serve + curl /v1/club              → HTTP 200, sin ruido nuevo al cerrar
```

### La ronda de arreglos de A-6, y qué enseñó del método

**Cinco hallazgos, un solo cambio de comportamiento.** El diff lo dice entero: quitando comentarios, lo que
cambia son **cuatro sitios de `ProblemMiddleware.swift`** y nada más. Todo lo demás es documentación puesta al
día y un fichero de tests nuevo. Lo que se hizo, por hallazgo:

| Hallazgo | Qué se tocó | Comportamiento |
|---|---|---|
| **H-43** | `ProblemMiddleware.swift` | **sí** — el motivo verdadero en el cuerpo y en el log |
| **H-40** | `ClubHandler.swift`, `IngestionHandler.swift` | no — dos comentarios que decían lo contrario de lo medido |
| **H-42** | `TenantResolutionMiddleware.swift`, `FluentTenantUnitOfWork.swift` | no — *"y no otro"* pasa a **puerta y cinturón** |
| **H-41** | `IngestClubCalendars.swift` | no — el `TODO(§7)` que faltaba, con su porqué |
| **H-44** | `Configure.swift` | no — el aviso puesto donde se rompería |

**La válvula de la regla 2 se disparó, y por segunda vez la conclusión era la contraria.** Cinco ficheros en
**cinco *targets*** es exactamente *"toca más de un target → mini-fase"*. Pero el corolario que dejó escrito la
ronda de A-5 —*"antes de aplazar por «toca muchos sitios», ir a mirar cuántas **líneas** son"*— manda otra
cosa: no hay API pública que cambie, ni dobles que mover, ni comportamiento nuevo salvo en un fichero. **La
válvula cuenta *targets* porque los targets suelen medir el riesgo; cuando no lo miden, hay que medirlo
directamente.** Nota para quien la aplique la próxima vez: *"cuántos targets"* es un indicador, no el criterio.

**Y la lección de la ronda, que es de método y se pagó dentro de ella.** El arreglo de H-43 **pasó su primer
test y no arregló la mitad que importaba**: se reflejaba el `ServerError` en vez de lo que lleva dentro, y un
envoltorio describe a su contenido con `String(describing:)` — o sea, exactamente el enmascaramiento del que
se venía huyendo. El cuerpo salía bien porque esa rama ya desenvolvía; el **log**, no. Lo delató la salida de
los propios tests: dos líneas seguidas con el mismo *"Generic description…"*. La regla que se añade:
**cuando un hallazgo dice «se pierde en las dos salidas», son dos caminos y hacen falta dos aserciones** — una
sola da un verde que no significa lo que parece, que es el tema recurrente de este libro (regla 4 de §3, H-11).

**La mutación que lo cazó no existía al empezar.** Anular el desenvuelto de `rootCause` **sobrevivía** a la
batería, y era *"falta un test"* de los legítimamente difíciles: el log está fuera del arnés por construcción,
como los códigos de salida de H-39. Resultó que no —`APIHandler` ya inyecta su `Logger` por este mismo motivo
(H-27) y `CapturingLogHandler` ya existía en el *target*—, así que se mató sustituyendo `app.logger` antes de
la petición, **sin tocar código de producción**. Nota para A-7, que iba a heredar esto como categoría: *antes
de aceptar que algo está fuera del arnés, mirar si el proyecto ya resolvió el caso hermano.*

**Las cuatro mutaciones, todas cazadas:**

```
1. el `case ServerError` no desenvuelve        → H-40 rojo (el error sale como INTERNAL)
2. los 5xx vuelven a `causeDescription`        → H-43 rojo (no hay 42P01 en el cuerpo)
3. la guarda de §6.1 nunca rechaza             → H-42 rojo (no lanza y se esperaba)
4. `rootCause` no desenvuelve                  → sobrevivía; con el test del log, H-43 rojo
```

**Cómo quedó la batería:** **297 tests** (293 + 4) con `REQUIRE_DB=1`, y `club_atleti` intacto — los tests
corren contra `tfm_test`, y el tenant de usar y tirar del bloque se borró al cerrar.

### Lo que A-7 entrega · el arnés propuesto, y qué se mide en vez de leer

**El bloque no arregla nada** (regla 2). Lo que deja es la propuesta y las mediciones que la sostienen.

**Las tres condiciones que A-2 le dejó apuntadas, revisadas — y una de las tres no se puede cumplir.**

| Condición heredada | Veredicto de A-7 |
|---|---|
| **`$? == 0`** | ✅ **es la única señal fiable de la CLI.** Medido en los tres estados: base arriba → `0`; base parada sin guarda → `0` *(el verde falso de H-07)*; base parada con `REQUIRE_DB=1` → `1` |
| **No leer el resumen de texto** | ✅ **confirmado y agravado.** No es solo que no haya un renglón final: es que los seis que hay son **idénticos en texto y en recuento** corriendo y omitiendo. Y con fallos el último renglón **cambia de forma** —pasa a ser la lista de *targets* rotos—, así que `tail -1` no vale ni para leer el resultado ni para detectar que no lo está leyendo |
| **`skipped == 0`** | ❌ **inalcanzable, y habría dado un CI rojo para siempre.** Con la base arriba **siempre** hay un omitido: el canario, que está fuera de la batería por diseño (§5.5). La condición correcta no es un número —un `== 1` caducaría con el segundo canario— sino **por motivo**: el XML trae el texto del `<skipped>`, así que se exige *"ningún omitido cuyo motivo hable de Postgres"* |

**Lo que lo hace posible, y es el hallazgo instrumental del bloque: `swift test --xunit-output` existe.**

```sh
REQUIRE_DB=1 swift test --xunit-output informe.xml     # emite informe-swift-testing.xml
```

Un solo XML con los **seis** *targets* como `<testsuite …>`, cada uno con `tests`, `failures`, `errors` y
`skipped`; los **297** `<testcase>`; y el omitido con **su motivo dentro**
(`<skipped>canario: exige FEDERATION_LIVE=1 y conexión a internet</skipped>`). Con eso las tres condiciones se
comprueban sin mirar una línea de texto, y el recuento —que §6-bis pide *"al día"*— se cuenta en vez de
sumarse a mano.

**El *workflow* propuesto** (montarlo es decisión de despliegue, y va con el cron):

| Paso | Por qué gana su sitio |
|---|---|
| Postgres **como servicio** + `REQUIRE_DB=1` | Sin la variable, un *runner* sin base da un verde de 0,002 s. `CI` la exportan GitHub Actions y GitLab solas, así que la guarda se activaría igual — pero **ponerla explícita** es lo que hace que el *workflow* se lea |
| `swift build` | **Medido**: una operación en el `filter` sin *handler* rompe el *build* (`APIHandler does not conform to APIProtocol`). El trinquete de `D-69` lo hace cumplir el compilador (H-49) |
| `redocly lint` **con versión fijada y `redocly.yaml`** | Hoy corre con el conjunto de reglas por defecto de `@latest`, así que *"válido"* puede cambiar sin que nadie toque el *spec*. Y necesita `setup-node`: `npx` no existe en un shell no interactivo (H-49) |
| **El canario, aparte y programado** | No puede ir en cada *push*: habla con internet y §5.5 explica que mezclarlo estropea las dos señales. Es el mismo deber de despliegue que el cron (`D-87`) |

**Y la otra mitad del bloque: la calidad de los tests, con el instrumento correcto.** Las tres cosas que §5
mandaba buscar salieron así:

| Qué se buscaba | Resultado |
|---|---|
| **Tests que no puedan fallar** | **Ninguno.** Los 297 `@Test` se analizaron uno a uno por cuerpo: **cero** sin `#expect`/`#require`/`expect(throws:)`. Y los dos testigos de tipo que el README defiende **no** son tautologías: `step2NeverReachesAnUnlinkedOwnTeam` afirma `outcome == .unmatched`, que se pone rojo el día que alguien cambie `TeamOwnership` por un `Bool` más un opcional — que es exactamente lo que el comentario dice que protege |
| **Aserciones sobre el doble en vez de sobre el efecto** | **Ninguna que lo sea de verdad.** Hay **15** tests que solo afirman sobre el doble, y en catorce **la llamada es el efecto**: cuando la regla es *"el antirrebote no lo pide"*, no pedirlo **es** la conducta y no hay fila que mirar. El decimoquinto —`una competición que falla no detiene el recorrido`— afirma solo la mitad *"se continúa"*, y la otra mitad de `D-86` la afirma `el recorrido dice cuál falló y por qué` justo debajo: es un reparto, no un hueco. Los dobles además **guardan entidades** (`IngestionStore`), así que el nivel 2 puede afirmar el efecto y lo hace |
| **Caminos de error sin test** | Aquí está todo lo que el bloque encontró, y **no lo encontró leyendo**: H-45 y H-46. `FederationError` es, al contrario de lo que el bloque sospechaba, **el mejor cubierto**: sus cuatro casos se citan en `Tests/` (4, 8, 2 y 2 veces). Su problema es otro y ya está escrito: en producción no los distingue nadie (**H-15**, acotado a F10 por A-6) |

**Ocho mutaciones, cuatro cazadas y cuatro supervivientes** — y las cuatro que sobreviven son los dos
hallazgos del bloque más el que le pone borde a la categoría:

| # | Se rompe | Resultado |
|---|---|---|
| **M0** | `requireSameSource` no opina (la guarda del nombre, `D-84`) | **cazada** por dos tests → **H-47** |
| **M1** | `databaseUnavailable` → otro caso (`IngestClubCalendars:71`) | **sobrevive** → **H-45** |
| **M1b** | el `break` de `IngestCommand:148` se dispara siempre | **cazada** — el hueco es de un lado |
| **M1c** | la sonda del ámbito 1 desaparece entera | **sobrevive** → **H-45** |
| **M2** | `unknownSeason` → otro caso | **cazada**, y por un test **de nivel 4**: el 404 |
| **M3** | `runNotRecorded` → `databaseUnavailable` | **sobrevive** → **H-46** |
| **M4** | `competitionNotFound` → `seasonNotFound` (404 → 500) | **sobrevive** → **H-46** |
| **M5** | `seasonNotFound` → `competitionNotFound` (500 → 404) | **sobrevive** → **H-46** |
| **M6** | `Task { }` → `Task.detached { }` | **sobrevive**, y con razón: programa equivalente → **H-48** |

**La lectura que ordena las cuatro supervivientes, y es la que el bloque venía a dar.** A-5 y A-6 le pasaron
el patrón *"la garantía depende de repetirlo a mano"* pidiéndole que decidiera si era una categoría de test
que falta. La respuesta es que **no es una categoría: es un síntoma de haber medido con el instrumento
equivocado**, y el libro lo ha hecho con los dos filos. A-6 encontró el primero —`grep` dice que algo está,
no que haga lo que dice— y H-47 encuentra el segundo, que produce hallazgos falsos en vez de tranquilidad
falsa: el cruce por nombre señaló seis casos *"sin test"* y la mutación desmintió tres. **Lo único que
contesta *"¿lo prueba algo?"* es romperlo y mirar** — y contestado así, lo que de verdad falta son dos cosas
concretas y baratas, no una categoría.

**La ronda se cerró con la regla 4, desde la CLI, y con el árbol limpio tras cada mutación:**

```
REQUIRE_DB=1 swift test  →  297 tests (7+60+51+105+50+24), 0 fallos, 1 omitido (el canario)
                            los dos targets de BD: 5,495 s y 2,702 s
```

**Ningún test de BD se omitió**, y esta vez el testigo es doble: las duraciones por la vía de H-07, y el
`skipped` del XML, que dice **cuál** se omitió y por qué.

### La ronda de arreglos de A-7, y qué enseñó del método

**Dos hallazgos, uno arreglado entero y el otro por mitades — y la válvula de §3 regla 2 disparó por tercera
vez, esta vez con el hermano ya identificado.**

- **H-46, cerrado.** Cinco casos de `ApplicationError` pasan de estar afirmados **por tipo** a estarlo **por
  caso**: tres tests nuevos y dos aserciones fortalecidas. La forma es
  `#expect { … } throws: { guard case … else { return false }; return true }`, que fija el caso **sin** fijar
  los valores asociados — importa que sea `runNotRecorded` y no cuál fue el `reason`, que es un diagnóstico.
- **H-45, cerrado por el lado que lanza y elevado por el lado que empareja.** El test de la sonda del ámbito 1
  costó ocho líneas porque **A-3 ya había dejado el instrumento**: `DatabaseSwitch` y `SwitchableUnitOfWork`
  existían; lo único que faltaba era bajar el interruptor **antes** de empezar en vez de a mitad, que es la
  diferencia entre los dos frenos de `D-86`.

**Siete mutaciones, siete cazadas** — las cinco que A-7 encontró vivas, más dos que el bloque no había
probado y que las aserciones fortalecidas destaparon:

| Se rompe | Antes | Lo caza ahora |
|---|---|---|
| `databaseUnavailable` → otro caso (`IngestClubCalendars:71`) | **sobrevivía** (M1) | `si la base no está al leer el plan, se dice con su nombre` |
| la sonda del ámbito 1 desaparece entera | **sobrevivía** (M1c) | el mismo — sale el error crudo del ámbito, no el caso |
| `runNotRecorded` → `databaseUnavailable` | **sobrevivía** (M3) | `una pasada que sí se escribió no se registra como fallida (H-24)` |
| `competitionNotFound` → `seasonNotFound` (la pasada) | **sobrevivía** (M4) | `una competición que ya no está da su propio caso` |
| `seasonNotFound` → `competitionNotFound` (la pasada) | **sobrevivía** (M5) | `una competición cuya temporada no está da el caso del schema roto` |
| `competitionNotFound` → otro caso (**el plan**) | **no se había probado** (M7) | `si una de las competiciones pedidas no existe, no se sincroniza ninguna (D-84)` |
| `unknownSeason` → otro caso (**el plan**) | solo el 404 de nivel 4 (M2) | ahora **también** el nivel 2: `una seasonId que no existe no cae a la vigente` |

**Los tres tests nuevos llegaron en verde**, que es deuda declarada (Plan §5.1), así que la garantía la da la
mutación — el mismo trato que las rondas de A-2 y A-5.

**Y la lección de método de la ronda, que es la de A-7 aplicada a sí misma.** Al fortalecer las dos
aserciones del plan aparecieron **M7 y M8**, dos mutaciones que el bloque **no había llegado a probar** porque
su inventario partió del cruce por nombre de caso —y `competitionNotFound` *"tenía citas en `Tests/`"*, solo
que en otro sitio—. O sea que **H-47 se cobró dentro de su propia ronda**: el instrumento que produce
hallazgos falsos produce también **puntos ciegos**, y los dos por la misma razón. La regla práctica que queda:
**cuando se fortalece una aserción, hay que mutar lo que afirma, no solo lo que la trajo.**

**Lo que la válvula elevó, y por qué esta vez no se discutió.** Probar el `break` de `IngestCommand.swift:148`
exige **una API pública nueva** —o una unidad de trabajo inyectable en `ingest`, o la regla extraída— y eso es
literalmente la definición de mini-fase de la regla 2. La diferencia con las dos veces anteriores es que aquí
**la salida ya está elegida y tiene precedente en el mismo fichero**: `incomplete(outcomes)` se separó de `run`
*"porque es la regla —no el `print`— y probarla no puede exigir montar una consola"*. Extraer
*"¿este resultado detiene el recorrido?"* con el mismo criterio pone el segundo freno de `D-86` bajo el arnés
sin tocar nada más. **Es H-48 aplicado en caliente**: antes de aceptar que algo no se puede probar, se buscó
el hermano, y estaba a catorce líneas.

**La ronda se cerró con la regla 4, desde la CLI:**

```
REQUIRE_DB=1 swift test  →  300 tests (7+60+51+105+53+24), 0 fallos, 1 omitido (el canario)
                            los dos targets de BD: 5,704 s y 2,755 s
```

**Y la cifra que esta ronda movió se corrige en el acto** (§2, excepción documental): de **297** a **300**
—tres tests de nivel 2—, así que `AGENTS.md` y las dos apariciones del `README` quedan al día. Las de **este**
fichero no se tocan: son la foto del día en que se midieron.
