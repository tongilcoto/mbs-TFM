...
5. **Heredado de A-6, y es la cuarta instancia del mismo patrón en su forma más barata** (H-40, H-42). A-6
   ...
   haga lo que dice**. Dos huecos concretos, los dos de una petición:
   - **Ningún test cubre que un error lanzado desde un *handler* llegue traducido.** El `case ServerError`
     de `ProblemMiddleware:232-239` existe exactamente para eso —lo pidió un test de F6, el del parámetro
     obligatorio ausente— y el camino *"un `ApplicationError` escapa del *handler* y sale con su código y su
     `detail`"* no lo afirma nada. Es la premisa de la que cuelgan los `do/catch` de los cuatro *handlers*
     (H-40) y lo que F10 va a leer.
   - **`tenantMismatch` no tiene ni un test**, y es la guarda de aislamiento entre clubes de §6.1 (H-42).
     Hoy además es inalcanzable, así que un test tendría que construir el actor a mano — que es precisamente
     lo que lo hace escribible **antes** de que llegue JWKS y no después.
...
| **A-6** · Las costuras de §7 | ● **auditado** — **cero S1**: las costuras están y poner §7 en marcha toca **cinco ficheros, una línea cada uno**, sin tocar ni un *handler*. Lo que falla no son las costuras sino **lo escrito encima**: dos comentarios de *handler* afirman que `ProblemMiddleware` no los alcanza (falso, medido) y la guarda de §6.1 compara un valor consigo mismo. Pendiente su ronda de arreglos | 2026-09-13 | H-40 · H-41 · H-42 · H-43 · H-44 (+ H-15 y H-34, confirmados y con destino: los dos aterrizan en H-43) |
| **A-7** · El arnés | ○ pendiente | — | H-07 (desde A-0) · la nota de cierre de A-2 · las **tres condiciones** que le deja la ronda de A-2: `$? == 0`, `skipped == 0` y **no leer el resumen de texto**, que no existe · y de A-4: **`DetachedBackgroundWork` no lo toca ningún test**, que es código fuera del alcance del arnés por construcción · y de A-5: **`H-38`**, que es la tercera vez que el libro encuentra *"la garantía depende de repetirlo a mano"* (H-17, H-26, H-38) — conviene que A-7 lo lea como **patrón**, no como tres hallazgos · y de A-6, **dos huecos concretos del arnés y una cuarta instancia del patrón**: (a) **ningún test cubre que un error lanzado desde un *handler* llegue traducido** —el `case ServerError` de `ProblemMiddleware:232-239` se escribió en F6 para eso y quien lo demuestra es un `curl`, no la batería (H-40)—; (b) **`tenantMismatch` no tiene ni un test** y es la guarda de aislamiento entre clubes de §6.1 (H-42); y (c) el patrón de H-38 aparece **en su forma más barata**: las tres costuras buenas las demostró `grep`, y las tres afirmaciones falsas solo se cayeron **ejecutando el servidor** — *lo que `grep` demuestra es que algo está, no que haga lo que dice* |
