...
struct MigrationIntegrityTests {
    ...
    func revertIsAFaithfulRoundTrip() async throws {
        try await Self.withApp { app in
            ...
            //
            // **14 desde F7**: tres de `standing_rows` —`position`,
            // `previous_position` y el de los siete contadores— y el de `kind` en
            // `ingestion_runs`. El número sube con cada fase y **eso es lo que se
            ...
            // (`D-92`).
            #expect(before.filter { $0.hasPrefix("c chk_") }.count == 14,
