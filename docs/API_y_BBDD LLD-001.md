...
| **LeagueScorer** (Goleador de liga)               | `competition_id`, **`federation_player_id`**, `full_name`, `team_label`, `goals`, `rank?`, `synced_at?`                                                                                                     | **Ingerida de la API de la liga** (§3.7); no ligada a `Player`; solo lectura. **Se ingiere, no se calcula** ([D-09]): el ranking incluye rivales, de los que no hay plantilla — `full_name` y `team_label` son **texto del proveedor**, no claves, y no se emparejan con `Player` ni con `Team`. **Estado vigente único, no *snapshot* por jornada** (a diferencia de `StandingRow`): el *upsert* lo pisa en cada sincronización, sin histórico ni `PREV`. **`federation_player_id`** es la **clave de *upsert*** ([D-93]) y es **obligatorio**, al revés que sus tres hermanas `federation_team_id` / `federation_club_id` / `federation_match_id`: una fila de esta tabla la escribe la ingesta o no existe —no hay `POST` ([D-21])—, así que no tiene el estado intermedio que hace anulables a aquéllas. Lo publican **las dos** federaciones (`codigo_jugador` y `codjugador`; 426/426 filas únicas y no vacías en los volcados, [Anexo RFFM §F.19], [Anexo FCF §C.10.7]) y **no viaja en el DTO**, igual que `synced_at`. **`rank`** anulable —no todos los proveedores lo publican— y **se respeta el suyo** en vez de recalcularlo: los criterios de desempate son ajenos. **Sin *fallback*** ([D-48]): si la federación no lo publica, no hay nada que calcular y la tabla queda vacía. **`synced_at`** es marca del *upsert*, y con ella ésta es **la única tabla de la salida de la ingesta que borra filas** ([D-94]): al ser estado vigente y no histórico, un goleador que el proveedor deja de publicar se quedaría para siempre mezclado con los vivos. Se retiran las filas **de esa competición** cuyo `synced_at` sea anterior al de la pasada. **No viaja en el DTO** ([D-29])                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **En `TeamRegistration` el `UNIQUE` va con `NULLS NOT DISTINCT`** (`team_id`, `season_id`, `competition_id`)
  ([D-68] enmienda), porque la tercera columna **es anulable**: sin esa cláusula, dos filas
  `(equipo, temporada, NULL)` no colisionarían y el mismo equipo saldría dos veces en la portada. Es
  literalmente la trampa que la clave de `Team` ya se comió en F5, y se paga una vez.
- **Y la coherencia entre `competition_id` y `season_id` es una FK compuesta, no una guarda**:
  `(competition_id, season_id) → competitions(id, season_id)`, lo que exige un `UNIQUE(id, season_id)` en
  `competitions` que no cuesta nada. Sin ella, nada impediría inscribir un equipo en una competición de otra
  temporada; con ella, **no es representable** ([D-61]).
- *(Antes de la enmienda)* **un `UNIQUE` normal bastaba** (`team_id`, `season_id`) ([D-68]): las dos columnas eran
  ...
  **nunca al revés**. Es la cascada que hay que declarar explícitamente, o la purga de [D-24] se llevaría los
  equipos del club.
- **El subárbol de `Season` cuelga con `ON DELETE CASCADE`, empezando por `Competition`** ([D-73]). Es el
  mecanismo con el que §5.4 ejecuta la **purga** (`?cascade=true`), igual que en `TeamRegistration`. La
  contrapartida hay que tenerla presente: el borrado **normal** —que solo debe tener éxito sin dependientes—
  ya no lo protege el esquema sino **la guarda del caso de uso**, que devuelve 409. Por eso los repositorios
  de `Season` y `Competition` **no exponen `delete`** mientras ese caso de uso no exista.
...
- **Value Objects (VO):**
  ...
  - **Valores con invariante** — p. ej. `MatchResult` (`homeScore`/`awayScore ≥ 0`), `ShirtNumber`,
    `SanctionBracket` (`yellowFrom ≤ yellowTo`) y `SeasonLabel`, del que además **se derivan** `startDate` y
    `endDate` en vez de almacenarse ([D-71]). Se construyen por *init* que **valida y lanza**; un VO que
...
- **En `Competition`, la FK a `seasons` va con `.references(..., onDelete: .cascade)`** ([D-73]), y el
  `UNIQUE` de §3.5 es `(season_id, federation_group_id)`. Los tres `CHECK` de enumerado —`modality`,
  `gender`, `age_category`— se derivan de `Enumerations.swift` vía `sqlValueList`, que es genérico sobre
  `CaseIterable where RawValue == String`: un enumerado nuevo lo hereda sin escribir nada.
...
- **CI/CD** — *build* en CI/builder remoto, migraciones, despliegue; ejecución de la pirámide de §8.1 en CI
  ...
  - **F1 no remide, y el motivo es el propio [D-69].** El coste del generador escala con el **alcance
    implementado**, no con el tamaño del *spec*, y F1 **no añade ninguna operación al `filter`**: entrega
    dominio y persistencia, sin HTTP. Las 543 líneas generadas son exactamente las mismas que en F0, así que
    lo que esta tabla vigila —el crecimiento del plugin por fase— no se ha movido. Lo que crece es código
    nuestro (siete ficheros). **La próxima remedida toca en la primera fase que amplíe el `filter`**, que por
    el plan es F10.
...
12. ~~**La terna *(equipo, temporada, competición)* no la sirve ninguna lectura.**~~ **RESUELTA — en el
    modelo, no en la consulta.** La inscripción del club pasa a llevar la competición
    (`TeamRegistration(team_id, season_id, competition_id?)`, [D-68] enmienda), así que la portada es un
    ...
    > sistema **acaba de escribir** que ese equipo va con esa competición y no lo puede leer. Ese hueco es el
    > mismo que creó `TeamRegistration` en el eje de la temporada, y se cierra con el mismo argumento.

    **El planteamiento original.** Es la **pantalla principal**
...
[Anexo RFFM §F.19]: ./API_y_BBDD%20LLD-Anexo-Federacion-Madrid-RFFM.md
